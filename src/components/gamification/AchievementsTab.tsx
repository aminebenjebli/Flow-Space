    ...
    const fetchAchievements = async () => {
      try {
        ...
        const mappedAchievements: Achievement[] = userAchievements.map((achievement) => ({
          ...
          unlocked: achievement.progress >= achievement.achievement.targetValue,
        }));
  ...
  if (achievements.length === 0) {
    return (
      <div className="p-4 text-gray-400 text-center">
        ...
        <p className="text-sm">Complete tasks to earn achievements and level up!</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {achievements.map((achievement) => (
        <div
          key={achievement.id}
          className={`p-4 rounded-lg text-white ${achievement.unlocked ? "bg-gradient-to-r from-purple-600 to-blue-500" : "bg-gray-800"}`}
        >
          <div className="flex items-center mb-2">
            <span className="mr-2 text-2xl">{achievement.icon}</span>
            <p className="font-medium">{achievement.name}</p>
            {achievement.unlocked && (
              <span className="ml-auto bg-yellow-500 text-xs px-2 py-0.5 rounded-full">Unlocked!</span>
            )}
          </div>
          ...
          <p className="text-xs text-gray-400">{achievement.progress}/{achievement.target}</p>
        </div>
      ))}
    </div>
  );
